...
def create_figure4():
    ...
    try:
        ...
        transform_info = pd.read_csv(os.path.join(current_dir, "transform_info.csv"))
        if len(transform_info) == 0:
            raise ValueError("Transform info is empty")
            
        # Load the grid cell data
        cell_data = pd.read_csv(os.path.join(current_dir, "grid_cell_data.csv"))
        if len(cell_data) == 0:
            raise ValueError("Cell data is empty")
            
        # Load the country counts
        country_counts = pd.read_csv(os.path.join(current_dir, "country_counts.csv"))
        if len(country_counts) == 0:
            raise ValueError("Country counts are empty")
            
    except Exception as e:
        print(f"Error loading pre-processed data: {e}")
        print("Please run process_geographic_origins_data.py first to generate the required CSV files")
        sys.exit(1)
    
    # Load the background map image
    img_path = os.path.join(current_dir, "British_Isles_map_showing_UK,_Republic_of_Ireland,_and_historic_counties.svg.png")
